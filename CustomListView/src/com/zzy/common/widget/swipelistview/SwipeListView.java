package com.zzy.common.widget.swipelistview;
...
public class SwipeListView extends ListView {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mIsHorizontal) {
				if (mIsShown && mPreItemView != mCurrentItemView) {
					/**
					 * 情况二： 这时候左右滑动另外一个item,那个右边布局显示的item隐藏其右边布局
					 * <p>
				...
				if (mIsShown) {
					/**
					 * 情况三： 这时候上下滚动ListView,那么那个右边布局显示的item隐藏其右边布局
					 */
					hiddenRight(mPreItemView);
				}
			}
		...
		case MotionEvent.ACTION_CANCEL:
			...
			if (mIsShown) {
				/**
				 * 情况四： 这时候左右滑动当前一个item,那个右边布局显示的item隐藏其右边布局
				 */
				hiddenRight(mPreItemView);
			}
			...
			if (mIsHorizontal != null && mIsHorizontal) {
				if (mFirstX - lastX > mRightViewWidth / 2) {
					...
					/**
					 * 情况五： 向右滑动item,且滑动的距离超过了右边View的宽度，隐藏
					 */
	...
	public Boolean getmIsHorizontal() {
