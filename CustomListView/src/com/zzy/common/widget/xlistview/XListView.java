...
public class XListView extends ListView implements OnScrollListener {
	...
	// -- header view
	protected AbsListViewHeader mHeaderView;
	...
	// -- footer view
	protected AbsXListViewFooter mFooterView;
	...
	public void startRefresh() {
		if (mPullRefreshing == false) {
			...
			mHeaderView.setState(SimpleXListViewHeader.STATE_REFRESHING);
			int finalHeight = mHeaderViewHeight;
			mScrollBack = SCROLLBACK_HEADER;
			mHeaderView.setVisiableHeight(finalHeight);
			mScroller.startScroll(0, 0, 0, finalHeight, 0);
			invalidate();
		}
	}
	...
	protected void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
			if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
				mHeaderView.setState(SimpleXListViewHeader.STATE_READY);
			} else {
				mHeaderView.setState(SimpleXListViewHeader.STATE_NORMAL);
			}
		}
		...
		default:
			...
			if (getFirstVisiblePosition() == 0) {
				...
				if (mEnablePullRefresh
					...
					mPullRefreshing = true;
					mHeaderView
							.setState(SimpleXListViewHeader.STATE_REFRESHING);
					if (mListViewListener != null) {
